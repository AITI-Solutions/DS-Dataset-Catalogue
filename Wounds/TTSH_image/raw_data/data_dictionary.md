...
This folder holds 26 per-batch CSVs (`batch_01.csv` … `batch_26.csv`) — the same annotation-index files as `Wounds/TTSH_image:overlay`.

> **Finding:** every `batch_NN.csv` here is **byte-for-byte identical** (same MD5 checksum, same row/column counts) to its counterpart in `TTSH_image:overlay`. This is the same wound/image annotation table, just placed alongside both the raw-image and overlay-image folders — it is not raw-image-specific metadata. Worth confirming whether that duplication is intentional (a convenience copy so each image folder is self-describing) or an artifact of how the export was produced.

Because the content is identical, the field table and the batch-level differences are the same as documented in `Wounds/TTSH_image:overlay/data_dictionary.md` — reproduced here for completeness rather than requiring a cross-folder lookup.
...
## Remarkable difference vs. the overlay copy

Beyond the batch-to-batch variation above: this entire folder's 26 CSVs are **identical, checksum-for-checksum, to the 26 CSVs in `Wounds/TTSH_image:overlay`**. `ImageDetail.ImageFilename` here still points at the raw filename (no `Ov_` prefix) either way — the file itself doesn't distinguish raw vs. overlay context, so treat this and the overlay dictionary as describing one shared annotation table that happens to be stored in two places.
