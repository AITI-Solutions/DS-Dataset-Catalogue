...
**This folder's 26 files are byte-for-byte identical to the 26 files in `Wounds/TTSH_image:overlay`** — same checksums, same row/column counts, same content. It is the same annotation-index table, kept alongside both the raw-image and overlay-image folders rather than raw-image-specific metadata. See `Wounds/TTSH_image:overlay/data_overview.md` and `data_dictionary.md` (or this folder's own copy) for the full breakdown.

This is also the pre-merge source data: `Original Dataset (outputs)/new_path_metadata.csv` is built by combining these batch files, renaming batches to a canonical scheme, and standardizing image/overlay file paths.
...
| **Cloud Provider** | NA |
| **Platform** | Github |
| **Storage Location** | https://github.com/AITI-Solutions/Wound-Project/tree/main/TTSH_image/raw_data |
| **Storage Format** | CSV |
| **Region** | NA |
| **Database ** | NA |
| **Schema** | NA |
| **Table Name** | TTSH_image/raw_data |
| **Environment** | |
| **Compression** | NA |
---
